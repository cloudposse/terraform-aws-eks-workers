...
locals {
  ...
  map_worker_role = [
    {
      rolearn  = local.workers_role_arn
...
provider "kubernetes" {
  ...
  dynamic "exec" {
    for_each = local.kube_exec_auth_enabled && length(local.cluster_endpoint) > 0 ? ["exec"] : []
    content {
      api_version = "client.authentication.k8s.io/v1beta1"
      command     = "aws"
      args        = concat(local.exec_profile, ["eks", "get-token", "--cluster-name", try(var.cluster_name, "deleted")], local.exec_role)
    }
  }
}
